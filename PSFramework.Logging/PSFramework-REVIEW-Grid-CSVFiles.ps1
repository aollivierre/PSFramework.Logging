...
try {
    ...
    foreach ($logFile in $logFiles) {
        Write-EnhancedLog -Message "Found CSV log file: $($logFile.FullName)" -Level "INFO"
    }
...
foreach ($logFile in $logFiles) {
    ...
    foreach ($entry in $logEntries) {
        ...
        if ($entry.PSObject.Properties["Timestamp"]) {
            ...
            # Add the file name and cleaned message as new columns, exclude SortingID and Callstack for now
