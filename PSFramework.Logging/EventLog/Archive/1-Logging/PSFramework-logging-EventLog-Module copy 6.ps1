...
function Write-LogMessage {
    ...
    # Get the parent script name without the file extension
    # $parentScriptName = ($MyInvocation.PSCommandPath | Split-Path -Leaf) -replace '\..*$', ''

    # Check if a log with this name already exists
    # if (-not [System.Diagnostics.EventLog]::Exists($parentScriptName)) {
    #     # If the log does not exist, create a unique ID based on the current date and time
    #     $uniqueID = (Get-Date).ToString("yyyyMMddHHmmss")

    #     # Combine the unique ID with the parent script name to create the final log name
    #     $parentScriptName = $uniqueID + "-" + $parentScriptName  
    # }


    $uniqueID = (Get-Date).ToString("yyyyMMddHHmmss")
    $parentScriptName = $uniqueID + "-" + $parentScriptName 
    Write-Host "Final log name: $parentScriptName"
