...
#     # Get the unique base for the log name
...
function Manage-EventLogs {
    <#
    .SYNOPSIS
    Manages the creation or selection of an event log based on the parent script name.

    .DESCRIPTION
    The Manage-EventLogs function generates a unique log name, checks if an event log with the same parent script name already exists, and either uses the existing log or creates a new one.

    .PARAMETER ParentScriptName
    The name of the parent script to be used as part of the event log name.

    .EXAMPLE
    Manage-EventLogs -ParentScriptName "MyScript"
    Generates or retrieves an event log based on the parent script name.
    #>
    ...
    Process {
        try {
            # Generate the unique base for the log name
            Write-EnhancedLog -Message "Generating unique base for log name." -Level "INFO"
            $uniqueBase = Generate-UniqueBase
            Write-EnhancedLog -Message "Unique base generated: $uniqueBase" -Level "INFO"

            # Combine the unique base with the parent script name
            $logName = $uniqueBase + "-" + $ParentScriptName
            ...
            $logs = @()
            try {
                Write-EnhancedLog -Message "Retrieving existing logs that match the parent script name: '$ParentScriptName'." -Level "INFO"
                $logs = @(Get-WinEvent -ListLog * | Where-Object { $_.LogName -like "*$ParentScriptName*" })
                Write-EnhancedLog -Message "Retrieved $($logs.Count) logs that match the parent script name." -Level "INFO"
            }
            catch {
                Write-EnhancedLog -Message "Failed to access some logs due to insufficient privileges or another issue: $($_.Exception.Message)" -Level "WARNING"
                throw $_
            }

            # Select the first log from the array if it exists
            if ($logs.Count -gt 0) {
                $existingLog = $logs | Select-Object -First 1
                # If a log with the same parent script name exists, use that log
                if ($existingLog) {
                    $logName = $existingLog.LogName
                    Write-EnhancedLog -Message "Event log with the same parent script name already exists. Using existing log: '$logName'." -Level "INFO"
                }
            }
            else {
                Write-EnhancedLog -Message "No existing log found for parent script name: '$ParentScriptName'. A new log will be created with the name: '$logName'." -Level "INFO"
            }
...
function Construct-SourceName {
    <#
    .SYNOPSIS
    Constructs the event log source name by concatenating the log name and caller function.

    .DESCRIPTION
    The Construct-SourceName function takes in the log name and caller function, concatenates them with a hyphen, and returns the result as the event log source name.
    ...
    .EXAMPLE
    $sourceName = Construct-SourceName -LogName "MyLog" -CallerFunction "MyFunction"
    #>
...
function Initialize-EventLog {
    ...
    Process {
        try {
            ...
            try {
                ...
                $logName = Manage-EventLogs -ParentScriptName $ParentScriptName
                Write-EnhancedLog -Message "Manage-EventLogs function returned log name: '$logName'." -Level "INFO"
            }
            catch {
            ...
            try {
                ...
                $logName = Sanitize-LogName -LogName $logName
                Write-EnhancedLog -Message "Sanitized log name: '$logName'." -Level "INFO"
            }
            catch {
            ...
            try {
                ...
                $sourceName = Construct-SourceName -LogName $logName -CallerFunction $callerFunction
                Write-EnhancedLog -Message "Constructed source name: '$sourceName'." -Level "INFO"
            }
            catch {
            ...
            Write-EnhancedLog -Message "Performing pre-validation for log '$logName' and source '$SourceName'." -Level "INFO"
            try {
                $validationResult = Validate-EventLog -LogName $logName -SourceName $SourceName
            ...
            # Step 3: Manage Sources
            Write-EnhancedLog -Message "Managing event log sources for log '$logName' and source '$SourceName'." -Level "INFO"
            try {
                Manage-EventLogSource -LogName $logName -SourceName $SourceName -ValidationResult $validationResult
            ...
            # Step 4: Post-Validation (Optional)
            Write-EnhancedLog -Message "Performing post-validation for log '$logName' and source '$SourceName'." -Level "INFO"
            try {
                Validate-EventLog -LogName $logName -SourceName $SourceName -PostValidation
    ...
    End {
        Write-EnhancedLog -Message "Preparing to exit Initialize-EventLog function. Returning log name: '$logName'." -Level "INFO"
        ...
        try {
            ...
            if ($logName -isnot [string]) {
                Write-EnhancedLog -Message "Log name is not a string. Actual type: $($logName.GetType().Name)" -Level "ERROR"
                throw "Unexpected type for log name. Expected string."
            }

            Write-EnhancedLog -Message "Log name '$logName' is valid and will be returned." -Level "INFO"
        }
        catch {
    ...
    Process {
        try {
            ...
            try {
                ...
                $global:logName = Initialize-EventLog -SourceName $callerFunction -ParentScriptName $ParentScriptName
                Write-EnhancedLog -Message "Event log initialized with name '$global:logName'." -Level "INFO"
            
                # Sanitize the log name
                Write-EnhancedLog -Message "Sanitizing the log name for source '$callerFunction'." -Level "INFO"
                $global:logName = Sanitize-LogName -LogName $global:logName
                Write-EnhancedLog -Message "Sanitized log name: '$global:logName'." -Level "INFO"
            ...
            if (-not $global:logName -or $global:logName -is [System.Collections.Hashtable]) {
                Write-EnhancedLog -Message "The log name '$global:logName' is invalid or is a hashtable. Halting operation." -Level "CRITICAL"
                throw "Invalid log name: '$global:logName'"
            }
            ...
            try {
                Write-EnhancedLog -Message "Validating if the event log source '$global:logName-$callerFunction' exists in log '$global:logName'." -Level "INFO"
                
                # Check if the source exists and count occurrences
                $sourceExists = [System.Diagnostics.EventLog]::SourceExists("$global:logName-$callerFunction")
                if ($sourceExists) {
                    $sourceLogName = [System.Diagnostics.EventLog]::LogNameFromSourceName("$global:logName-$callerFunction", ".")
                    $sourceCount = @(Get-EventLog -LogName $sourceLogName -Source "$global:logName-$callerFunction").Count
                    
                    Write-EnhancedLog -Message "Event log source '$global:logName-$callerFunction' exists in log '$global:logName'. Number of occurrences: $sourceCount." -Level "INFO"
                }
                else {
                    Write-EnhancedLog -Message "Event log source '$global:logName-$callerFunction' does not exist in log '$global:logName'. Proceeding to create it." -Level "WARNING"
                }
            
                $DBG
                Write-EnhancedLog -Message "Writing log message to event log '$global:logName' for source '$global:logName-$callerFunction'." -Level "INFO"
                ...
                Write-EnhancedLog -Message "Failed to write log message to event log '$global:logName' for source '$global:logName-$callerFunction': $($_.Exception.Message)" -Level "ERROR"
                throw $_
            }
