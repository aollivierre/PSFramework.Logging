    ...
    Process {
        try {
            ...
            try {
                ...
                $global:sanitizedsourceName = Construct-SourceName -LogName $logName -CallerFunction $SourceName
            ...
            Write-EnhancedLog -Message "Performing pre-validation for log '$logName' and source '$global:sanitizedsourceName'." -Level "INFO"
            try {
                $validationResult = Validate-EventLog -LogName $logName -SourceName $global:sanitizedsourceName
            ...
            Write-EnhancedLog -Message "Managing event log sources for log '$logName' and source '$global:sanitizedsourceName'." -Level "INFO"
            try {
                Manage-EventLogSource -LogName $logName -SourceName $global:sanitizedsourceName -ValidationResult $validationResult
            ...
            Write-EnhancedLog -Message "Performing post-validation for log '$logName' and source '$global:sanitizedsourceName'." -Level "INFO"
            try {
                Validate-EventLog -LogName $logName -SourceName $global:sanitizedsourceName -PostValidation
...
function Write-LogMessage {
    ...
    Process {
        try {
            ...
            try {
                Write-EnhancedLog -Message "Validating if the event log source '$global:sanitizedsourceName' exists in log '$global:logName'." -Level "INFO"
                ...
                if ($sourceExists) {
                    ...
                    Write-EnhancedLog -Message "Event log source '$global:sanitizedsourceName' exists in log '$global:logName'. Number of occurrences: $sourceCount." -Level "INFO"
                }
                else {
                    Write-EnhancedLog -Message "Event log source '$global:sanitizedsourceName' does not exist in log '$global:logName'. Proceeding to create it." -Level "WARNING"
                }
            
                $DBG
                Write-EnhancedLog -Message "Writing log message to event log '$global:logName' for source '$global:sanitizedsourceName'." -Level "INFO"
                Write-EventLog -LogName $global:logName -Source "$global:sanitizedsourceName" -EntryType $entryType -EventId $eventID -Message $Message
                Write-Host "Logged message to '$global:logName' from source '$global:sanitizedsourceName'"
            }
            catch {
                Write-EnhancedLog -Message "Failed to write log message to event log '$global:logName' for source '$global:sanitizedsourceName': $($_.Exception.Message)" -Level "ERROR"
                throw $_
            }
