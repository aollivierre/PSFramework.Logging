...
function Generate-UniqueBase {
    $timestamp = (Get-Date).ToString("yyMMddHHmm")  # 10 characters timestamp
    $processid = $PID.ToString("D4")  # 4-digit process ID
    $randomPart = (Get-Random -Minimum 10 -Maximum 99).ToString("D2")  # 2-digit random number

    # Combine parts, ensuring the first 8 characters are unique
    $uniqueBase = $timestamp.Substring(6, 2) + $processid.Substring(0, 2) + $randomPart
