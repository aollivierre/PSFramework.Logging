...
function Initialize-EventLog {
    ...
    $sourceLogName = if ($sourceExists) { [System.Diagnostics.EventLog]::LogNameFromSourceName($SourceName, ".") } else { $null }
    ...
    elseif ($logExists -and $sourceExists -and $sourceLogName -ne $logName) {
        ...
        New-EventLog -LogName $logName -Source $SourceName
        Write-Host "Source '$SourceName' was associated with a different log ('$sourceLogName'). Recreated event log '$logName' with source '$SourceName'."
    }
    ...
    elseif ($logExists -and -not $sourceExists) {
        ...
        Write-Host "Event log '$logName' and source '$SourceName' already exist and are correctly associated."
    }
...
function Write-LogMessage {
    ...
    Write-EventLog -LogName $global:logName -Source $uniqueSource -EntryType $entryType -EventId $eventID -Message $Message
    Write-Host "Logged message to '$global:logName' from source '$uniqueSource'"
}
...
function ExampleFunction2 {
    Write-LogMessage -Message "This is a test message from ExampleFunction2." -Level "INFO"
}
