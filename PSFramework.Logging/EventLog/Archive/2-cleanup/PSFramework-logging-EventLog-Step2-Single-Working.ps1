function Remove-ModernEventLog {
    ...
    # Remove the associated registry key
    $regKeyPath = "HKLM:\SYSTEM\CurrentControlSet\Services\EventLog\$LogName"
    if (Test-Path $regKeyPath) {
        Remove-Item -Path $regKeyPath -Recurse -Force
        Write-Host "Registry key '$regKeyPath' deleted."
    } else {
...
# Example usage
Remove-ModernEventLog -LogName "PSFCustomlog"
